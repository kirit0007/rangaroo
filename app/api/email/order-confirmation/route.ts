...
export async function POST(request: Request) {
  try {
    ...
    const { order, customerEmail: paramEmail } = body || {};

    if (!order) {
      return NextResponse.json({ error: 'Order data is required' }, { status: 400 });
    }

    const recipientEmail = paramEmail || order.shippingAddress?.email || order.email || 'customer@example.com';
        ...
        <table align="center" border="0" cellpadding="0" cellspacing="0" width="100%" style="max-width: 600px; background-color: #ffffff; border-radius: 20px; overflow: hidden; box-shadow: 0 4px 15px rgba(0,0,0,0.05); border: 1px solid #ffe8d6;">
          ...
          <tr>
            <td style="padding: 30px 30px 10px 30px; text-align: center;">
              ...
              <p style="color: #6b7280; margin: 0; font-size: 14px;">We have received your payment and are getting your paint kit ready for dispatch.</p>
            </td>
          </tr>
    ...
    if (!response.ok) {
      ...
      return NextResponse.json({ error: data.message || 'Failed to send email via Brevo' }, { status: response.status });
    }
